...
const game = {
    ...
    collision: new Set(),
    generateShip(){
        for(let i = 0; i < game.optionShip.count.length; i++){
            for(let j = 0; j < game.optionShip.count[i]; j++){
...
const fire = (e) => {
    ...
    if(target.classList.contains('miss') || target.tagName !== 'TD' || game.shipCount < 1 ) return;
    show.miss(target);
